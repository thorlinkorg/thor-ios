...
//            indicator?.setValue(progress, forKey: "progress")
    ...
    func hide(_ animated: Bool, afterDelay delay: TimeInterval) {
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + Double(delay * Double(NSEC_PER_SEC)) / Double(NSEC_PER_SEC)) { () -> Void in
    ...
    override func draw(_ rect: CGRect) {
        ...
        let boxRect: CGRect = CGRect(x: round((allRect.size.width - mbsize.width) / 2) + CGFloat(self.xOffset), y: round((allRect.size.height - mbsize.height) / 2) + CGFloat(self.yOffset), width: mbsize.width, height: mbsize.height)
