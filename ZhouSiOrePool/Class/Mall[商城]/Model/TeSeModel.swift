...
class TeSeModel : NSObject, Codable ,NSCoding{
    ...
    init(fromDictionary dictionary: [String:Any]){
        ...
        specList = dictionary["specList"] as? String
        ...
        if let attributesList = dictionary["attributeList"] as? [[String: String]]{
            //整理列表为换行string
            var tempStr: String = ""
            let list = attributesList.compactMap({AttributeModel(fromDictionary: $0)})
            list.forEach { (model) in
                tempStr.append(model.attName + " :")
                tempStr.append(model.attValue + "\n")
                
            }
            attributeStr = tempStr
        }
    ...
    */
    @objc required init(coder aDecoder: NSCoder)
    {
        ...
        price = aDecoder.decodeObject(forKey: "price") as? Double ?? 0.0
         ...
         specList = aDecoder.decodeObject(forKey: "specList") as? String
    ...
    */
    @objc func encode(with aCoder: NSCoder)
    {
        ...
        if specList != nil{
            aCoder.encode(specList, forKey: "specList")
        }
        if stock != nil{
...
class AttributeModel: NSObject {
     ...
     */
    init(fromDictionary dictionary: [String:Any]){
        attValue = dictionary["attValue"] as? String
        attName = dictionary["attName"] as? String
    }
}
